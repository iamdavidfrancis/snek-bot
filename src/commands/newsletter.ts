...
import type { IPendingInvite } from '../services/db-schema.interface.js';
import type DBService from '../services/db-service.js';
import { InvitationDBStatus } from '../services/db-service.js';
import type MailgunService from '../services/mailgun-service.js';
import ServiceFactory from '../services/serviceFactory.js';

type Handler = (message: Message, args: string[]) => Promise<void>;
...
export default class Newsletter implements ICommand {
  ...
  public handler = async (message: Message, args: string[]): Promise<void> => {
    ...
    const argsv = args.splice(1);
    await this.subCommands.get(subCommand)!(message, argsv);
  };

  private readonly helpHandler = async (message: Message): Promise<void> => {
    const embed = new EmbedBuilder()
      ...
      .addFields(
        ...
        {
          name: Config.commandPrefix + 'plex-newsletter subscribe {email}',
          value: 'Subscribe to the plex newsletter with the provided email.',
        },
        {
          name: Config.commandPrefix + 'plex-newsletter unsubscribe {email}',
          value: 'Unsubscribe from the plex newsletter.',
        },
      )
      .setTimestamp()
      .setFooter({
  ...
  private readonly subscribeHandler = async (message: Message, args: string[]): Promise<void> => {
    ...
    if (otp) {
      if (status !== InvitationDBStatus.Unredeemed && status !== InvitationDBStatus.TooManyAttempts) {
        await message.reply(
          'This email is not currently pending. Please contact Franchyze#9084 if you believe this to be an error.',
        );
        return;
      }
      ...
      if (otp !== pendingInvite.otp) {
        await message.reply(
          'The submitted code is not valid. Please send the code exactly as it appears in the email.',
        );
        return;
      }
      ...
      if (pendingInvite.inviteExpirationTime && new Date() > pendingInvite.inviteExpirationTime) {
        await message.reply(
          `This code is expired. Please try again with \`${Config.commandPrefix}plex-newsletter subscribe ${email}\``,
        );
        return;
      }
      ...
      if (mailgunStatus) {
        await message.reply(
          'It looks like the email is already subscribed. Please contact Franchyze#9084 if you are not receiving the newsletter.',
        );
        return;
      }
      ...
      switch (status) {
        case InvitationDBStatus.TooManyAttempts:
          await message.reply(
            'Sorry, there have been too many attempts to add that email address. Please contact Franchyze#9084 for more information.',
          );
          return;
        ...
        case InvitationDBStatus.Invited:
        default:
  ...
  private readonly unsubscribeHandler = async (message: Message, args: string[]): Promise<void> => {
    if (args.length !== 1) {
      await this.invalidMessage(message);
      return;
    }
    ...
    await message.reply('Unsubscribed successfully.');
  };

  private readonly invalidMessage = async (message: Message) => {
    await message.reply('Sorry, that was not a valid command.');
  };

  private readonly performSubscribe = async (message: Message, email: string, isRedeem: boolean = false) => {
